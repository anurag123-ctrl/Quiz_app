...
const DeleteModel = ({ closeModel, id }) => {
    ...
    return ( <
        ...
        <
        motion.div className = "delete-model-main"
        initial = {
            { scale: 0.8, opacity: 0 } }
        animate = {
            { scale: 1, opacity: 1 } }
        ...
        p >
        Deleting this will result in loosing the file permanently and is not recoverable, Are you ok with it ?
        <
        /p> <
        div className = "delete-model-actions" >
        <
        button onClick = {
            () => onDeleteHandler(id) } > Yes < /button> <
        button onClick = {
            () => closeModel() } > No < /button> <
        /div> <
        /div> <
        /motion.div> <
        /div>
...
export const MyQuizzes = () => {
        ...
        return ( <
            > {
                model && < DeleteModel closeModel = { closeModel }
                id = { deleteId }
                />} <
                motion.div
                ...
                div className = "all-quiz-container" > {
                    Quiz.length === 0 ? ( <
                        p style = {
                            { color: "red" } } > Currently there are no quizes! < /p>
                    ) : (
                        Quiz.map((el, i) => ( <
                            Quizzes key = { el.id }
                            title = { el.title }
